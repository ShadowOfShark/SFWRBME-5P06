...
# only train classifier parameters (head)
...
def evaluate(model, loader, criterion, device, return_probs=False):
    """
    If return_probs=True, also return per-sample scores for ROC.
    """
    model.eval()
    running_loss = 0.0
    running_corrects = 0
    ...
    all_labels = []
    all_preds = []
    all_scores = []  # probability of class 0 ("Calculus")
    ...
    with torch.no_grad():
        for images, labels in loader:
            ...
            all_scores.extend(probs[:, 0].cpu().tolist())  # class 0
...
plt.xlabel("Epoch")
plt.ylabel("Loss")
plt.title("MobileNetV3-Small: Training and Test Loss")
plt.legend()
plt.grid(True)
...
plt.xlabel("Epoch")
plt.ylabel("Accuracy")
plt.title("MobileNetV3-Small: Training and Test Accuracy")
plt.legend()
plt.grid(True)
...
plt.title("MobileNetV3-Small: ROC Curve (Calculus as positive)")
plt.legend()
plt.grid(True)
plt.savefig(os.path.join("plots_mobilenet", "roc_curve_mobilenetv3.png"), dpi=300)
plt.close()

print("\nSaved MobileNet plots to 'plots_mobilenet'.")


# save model
os.makedirs("saved_models", exist_ok=True)
model_path = os.path.join("saved_models", "mobilenetv3_v2_calculus_vs_healthy.pth")
